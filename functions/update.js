...
export default async(req, res) => {
  ...
  const UPDATE_QUERY = `
    ...
    // Mutation to insert new email
  const INSERT_EMAIL_QUERY = `
    mutation addEmail($email: String!, $description: String!, $img_text: String!, $user: String!) {
      insert_email_one(object: {email: $email, description: $description, img_text: $img_text, user: $user}) {
        id
      }
    }
  `;
  ...
  try {
    ...
    const { data: newdata, error: insertError } =
      await nhost.graphql.request(INSERT_EMAIL_QUERY, {
        id: emailId,
        date: new Date(),
      });
